...
public class GithubSubmitTaskService implements SubmitTaskService {
    ...
    public AttemptDto submitTask(SubmitTaskRequest request, String owner, String accessToken) {
        ...
        try {
            ...
            TestSuiteEntity testSuite = attempt.getTestSuite();
            TestSuiteDto testSuiteDto = testSuiteEntityToTestSuiteDtoConverter.convert(testSuite);
            return new AttemptDto(attempt.getId(), attempt.getRunNumber(), attempt.getDetailsUrl(), String.valueOf(attempt.getResult()), attempt.getStudentTakenLab().getId(), testSuiteDto);
